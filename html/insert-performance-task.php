...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($subject_code && isset($_POST['performance_tasks'])) {
        foreach ($_POST['performance_tasks'] as $student_id => $weeks) {
            foreach ($weeks as $week => $score) {
                if ($score !== null && $score !== '') { // Only insert if a score is provided
                    ...
                    $checkQuery = "SELECT score FROM activities_performance_tasks 
                                   WHERE student_id = ? AND subject_code = ? AND term = '1st' AND week = ? AND task_type = 'Performance Task'";
                    ...
                    if ($existingScore === null || $existingScore != $score) {
                        ...
                        if ($stmt->execute()) {
                            ...
                            if ($studentResult && $studentResult->num_rows > 0) {
                                ...
                                $notificationMessage = "Your child, $studentName, got $score for Performance Task no. $week in $subjectName.";
                                $notificationQuery = "
                                    INSERT INTO notifications (parent_id, student_id, message, is_read, created_at, type)
                                    VALUES ('$parentId', '$student_id', '$notificationMessage', 0, NOW(), 'Performance Task')
                                ";
        ...
        echo "<script>
                alert('Subject code and at least one performance task score are required.');
                window.location.href = document.referrer;
              </script>";
    }
}
